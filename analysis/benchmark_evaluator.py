...
from convobench.scenarios import get_scenario
...
class BenchmarkEvaluator:
    ...
    def evaluate_model_performance(self, conversation: Dict[str, Any], target_agent: str) -> Dict[str, Any]:
        ...
        if scenario_id:
            ...
            if scenario:
                role = scenario.agent_a_role if target_agent == 'a' else scenario.agent_b_role
                goal = scenario.agent_a_goal if target_agent == 'a' else scenario.agent_b_goal
                
                scenario_context = f"SCENARIO: {scenario.name}\nDESCRIPTION: {scenario.description}"
                goal_context = f"ASSIGNED ROLE: {role}\nASSIGNED GOAL: {goal}"
                tool_criteria = "6. **Tool Usage**: Did the model use the provided tools correctly and effectively to achieve its goal? (If applicable)"
        ...
        for msg in messages:
            role_raw = msg['role'].replace('agent_', '').upper()
            role_label = f"Agent {role_raw}" if role_raw not in ["SYSTEM"] else "SYSTEM"
...
"""
        ...
        try:
            ...
            metrics["metrics_json"] = {
                ...
                "score_tool_usage": metrics.get("score_tool_usage", 0.0)
            }
